...
class cfg(cfg_common, cfg_dataset_default, cfg_model_vitad):
    ...
    def __init__(self):
        ...
        self.data.aug_transforms = [
            dict(type='RandomResizedCrop', size=(self.size, self.size), scale=(0.7, 1.0)),
            ...
            dict(type='GaussianBlur', kernel_size=(5, 9), sigma=(0.1, 2.0)),
            dict(type='ToTensor'),
            dict(type='Normalize', mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, inplace=True),
        ]
        ...
        self.optim.distill_opt.kwargs = dict(name='adamw', betas=(0.9, 0.999), eps=1e-8, weight_decay=self.weight_decay,
                                             amsgrad=False)

        # ==> trainer
        self.trainer.name = 'ViTADLGCTrainer'
