...
class cfg(cfg_common, cfg_dataset_default, cfg_model_rd):
    ...
    def __init__(self):
        ...
        # ==> loss (BYOL losses - NO negative samples!)
        self.loss.loss_terms = [
            dict(type='CosLoss', name='cos', avg=False, lam=1.0),
            # Changed: Use BYOLDenseLoss instead of DenseLoss
            dict(type='ClassAwareBYOLDenseLoss', name='dense', lam=1.0, use_spatial_matching=True),
            dict(type='SCLLoss', name='scl', lam=1.0, temperature=0.1),
        ]

        # Alternative: Use class-aware BYOL loss
        # dict(type='ClassAwareBYOLDenseLoss', name='dense', lam=1.0, use_spatial_matching=True),
