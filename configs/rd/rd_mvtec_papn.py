"""
Configuration for RDLGC with PAPN-style Contrastive Learning

This config replaces DenseCL with PAPN-MoCo loss for better fine-grained learning
"""
...
class cfg(cfg_common, cfg_dataset_default, cfg_model_rd):
    ...
    def __init__(self):
        ...
        self.model.kwargs = dict(
            ...
            dp=False,
            momentum=0.999  # MoCo-style momentum
        )
        ...
        # ==> trainer
        self.trainer.name = 'RDLGCTrainer'  # Use existing trainer
        ...
        self.trainer.scheduler_kwargs = dict(
            ...
            lr_min=self.lr / 1e2,
            warmup_lr=self.lr / 1e3, warmup_iters=-1, cooldown_iters=0,
        ...
        # ==> loss - PAPN configuration
        # Replace DenseLoss with PAPNMoCoLoss (keep name='dense' for trainer compatibility)
        self.loss.loss_terms = [
            dict(type='CosLoss', name='cos', avg=False, lam=1.0),
            dict(type='SCLLoss', name='scl', lam=1.0, temperature=0.1),
            dict(
                type='PAPNMoCoLoss',
                name='dense',             # Keep name='dense' for trainer compatibility
                feature_dim=1024,         # layer3 output dimension (ResNet)
                proj_dim=256,             # Projection dimension
                n_parts=5,                # Number of part prototypes
                queue_size=4096,          # MoCo queue size
                momentum=0.999,           # Not used (model has its own momentum)
                temperature=0.1,          # Temperature for InfoNCE loss
                enforce_orthogonal=False, # Orthogonal regularization
                ortho_loss_weight=0.1,    # Weight for ortho loss
                lam=1.0                   # Overall loss weight
            ),
        ]
        ...
        self.logging.log_terms_train = [
            ...
            dict(name='scl', suffixes=[''], fmt=':>5.3f', add_name='avg'),
            dict(name='dense', suffixes=[''], fmt=':>5.3f', add_name='avg'),  # PAPN loss (named 'dense')
        ]
