...
class ViTADLGCTrainer(BaseTrainer):
    def __init__(self, cfg):
        super(ViTADLGCTrainer, self).__init__(cfg)
    ...
    def forward(self):
        self.feats_t, self.feats_s, self.mid, self.feats_n, self.feats_proj, self.feats_n_aug, self.feats_proj_aug = self.net(self.imgs, self.aug_imgs)
    ...
    def optimize_parameters(self):
        ...
        with self.amp_autocast():
            ...
            loss_scl = self.loss_terms['scl'](self.mid, self.labels)
            loss_dense = self.loss_terms['dense'](self.feats_n,self.feats_proj, self.feats_n_aug, self.feats_proj_aug, self.labels)
            loss = loss_cos + loss_scl + loss_dense
        ...
        update_log_term(self.log_terms.get('cos'), reduce_tensor(loss_cos, self.world_size).clone().detach().item(), 1,
                        self.master)
        update_log_term(self.log_terms.get('scl'), reduce_tensor(loss_scl, self.world_size).clone().detach().item(), 1,
                        self.master)
        update_log_term(self.log_terms.get('dense'), reduce_tensor(loss_dense, self.world_size).clone().detach().item(), 1,
                        self.master)
